...
def main():
    ...
    print("=" * 60)
    print("  Living Epoch Alpha-2.5 — Behavioral Stress")
    print("  8750 cycles · Mid-Phase Regime Shifts")
    print("  Replay Challenge: seed_offset")
    print("=" * 60)
    print()
    sys.stdout.flush()
    ...
    if report.postmortem:
        ...
        print(f"    {'Phase'.ljust(18)} {'cycle'.ljust(6)}  eV          gO          cD          rD          rL")
        for i, ps in enumerate(pm.phase_snapshots):
            m = ps.metrics
            print(f"    [{i}] {ps.phase.value.ljust(18)} {str(ps.cycle).ljust(6)}"
                  f"  {m.entropy_velocity:.6f}  {m.governance_oscillation_velocity:.6f}"
                  f"  {m.confidence_drift_velocity:.6f}  {m.replay_divergence_velocity:.6f}"
                  f"  {m.recovery_latency_slope:.6f}")
        ...
        print()
        print("  ALPHA-2.5 SUCCESS CRITERIA")
        ...
        if phases:
            ...
            print(f"    max |replay_divergence|          : {max_rd:.6f}  (target > 0.0 — challenged)")
